...
import { Search, ChevronLeft, ChevronRight, Wallet, CreditCard, PiggyBank, TrendingUp } from 'lucide-vue-next'
  ...
  <div class="flex flex-col lg:flex-row min-h-screen">
    ...
    <div class="flex-1 pt-8 px-2 sm:px-4 lg:px-13 pb-10">
      <div class="max-w-7xl mx-auto w-full">
        ...
        <h1 class="text-xl sm:text-2xl lg:text-4xl font-bold mb-1 sm:mb-2 font-serif">Accounts Management</h1>
        <p class="text-muted-foreground text-xs sm:text-sm lg:text-base">
          Kelola dan pantau semua akun pengguna dalam sistem
        </p>
        <br />

        <!-- Stats Cards Section -->
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-6 mb-6 sm:mb-8">
          <Card class="p-2 sm:p-6">
            <div class="flex flex-row items-center justify-between text-left space-x-2 sm:space-x-3">
              <div>
                <p class="text-xs font-medium text-muted-foreground">Total Accounts</p>
                <p class="text-lg sm:text-3xl font-bold text-gray-900">{{ totalAccounts }}</p>
              </div>
              <div class="p-2 sm:p-3 bg-blue-100 rounded-full flex-shrink-0">
                <Wallet class="h-5 w-5 sm:h-8 sm:w-8 text-blue-600" />
              </div>
            </div>
          </Card>
          ...
          <Card class="p-2 sm:p-6">
            <div class="flex flex-row items-center justify-between text-left space-x-2 sm:space-x-3">
              <div>
                <p class="text-xs font-medium text-muted-foreground">Total Balance</p>
                <p class="text-lg sm:text-2xl font-bold text-gray-900">{{ formatCurrency(totalBalance) }}</p>
              </div>
              <div class="p-2 sm:p-3 bg-green-100 rounded-full flex-shrink-0">
                <PiggyBank class="h-5 w-5 sm:h-8 sm:w-8 text-green-600" />
              </div>
            </div>
          </Card>
          ...
          <Card class="p-2 sm:p-6">
            <div class="flex flex-row items-center justify-between text-left space-x-2 sm:space-x-3">
              <div>
                <p class="text-xs font-medium text-muted-foreground">Average Balance</p>
                <p class="text-lg sm:text-2xl font-bold text-gray-900">{{ formatCurrency(averageBalance) }}</p>
              </div>
              <div class="p-2 sm:p-3 bg-purple-100 rounded-full flex-shrink-0">
                <TrendingUp class="h-5 w-5 sm:h-8 sm:w-8 text-purple-600" />
              </div>
            </div>
          </Card>
          ...
          <Card class="p-2 sm:p-6">
            <div class="space-y-2">
              <p class="text-xs font-medium text-muted-foreground">Account Types</p>
              <div class="space-y-1">
        ...
        <div class="bg-white rounded-lg shadow overflow-x-auto">
          <div class="bg-white rounded-2xl shadow-md overflow-x-auto">
            <div class="px-3 py-3 sm:px-7 sm:py-4 border-b">
              <h2 class="text-base sm:text-lg font-serif">Accounts</h2>
                ...
                <TableBody>
                  <template v-for="account in accounts.data" :key="account.id">
                    <TableRow
                      ...
                      @click="toggleRowDetail(account.id)"
                    >
                      ...
                      <TableCell>
                        <div>
                          <div class="font-semibold text-xs">{{ account.user_name }}</div>
                          <div class="text-xs text-muted-foreground">{{ account.user_email }}</div>
                        </div>
                      </TableCell>
                      <TableCell class="truncate max-w-[12rem]" :title="account.budget_name">
                        {{ account.budget_name }}
                      </TableCell>
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                          ...
                          <div class="space-y-2">
                            <div><strong>Balance:</strong> 
                              <span :class="account.balance >= 0 ? 'text-green-600' : 'text-red-600'" class="font-semibold">
                                {{ formatCurrency(account.balance) }}
                              </span>
                            </div>
                            <div><strong>Interest Rate:</strong> {{ account.interest > 0 ? formatPercentage(account.interest) : 'No Interest' }}</div>
                            <div><strong>Minimum Monthly Payment:</strong> {{ account.minimum_payment_monthly > 0 ? formatCurrency(account.minimum_payment_monthly) : 'No Minimum Payment' }}</div>
            ...
            <div class="block lg:hidden">
              <div class="divide-y">
                <div
                  ...
                  :key="account.id"
                  class="py-3 px-2 flex flex-col gap-2"
                >
                  <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-2">
                      <component 
                        ...
                        class="h-4 w-4 text-muted-foreground" 
                      />
                      <span class="font-semibold text-sm">{{ account.name }}</span>
                    </div>
                    <Badge
                      class="px-2 py-1 text-xs rounded-full"
                      :class="getAccountTypeBadgeClass(account.type)"
                  ...
                  <div class="text-xs">
                    <Badge class="text-xs">Budget:</Badge> {{ account.budget_name }}
                  </div>
                  
                  <div class="flex justify-between items-center">
                    <div class="text-xs">
                      <Badge class="font-medium">Balance:</Badge>
                      <span :class="account.balance >= 0 ? 'text-green-600' : 'text-red-600'" class="font-semibold ml-1">
                        {{ formatCurrency(account.balance) }}
                      </span>
                    </div>
                  ...
                  <div v-if="account.interest > 0 || account.minimum_payment_monthly > 0" class="flex flex-wrap gap-2 text-xs">
                    <Badge v-if="account.interest > 0">
                      Interest: {{ formatPercentage(account.interest) }}
                    </Badge>
        ...
        <!-- No data fallback -->
        <div v-if="accounts.data.length === 0" class="text-center py-8 sm:py-12">
